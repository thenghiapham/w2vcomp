...
public class MMSkipgramEuclide extends SingleThreadWord2Vec {
    double alphaRatio = 0;
    public MMSkipgramEuclide(int projectionLayerSize, int windowSize,
    ...
    public void trainSentence(int[] sentence) {
        ...
        for (int wordPosition = 0; wordPosition < sentence.length; wordPosition++) {
            ...
            if (alphaRatio > 0 && jPerceptIndex!=-1) {
                ...
                for (int j = 0; j < projectionLayerSize; j++) {
                    a1error[j] += gradient
                        * (- negativeWeights1Images[jPerceptIndex][j] + 
                                weights0[wordIndex][j]);
                }

...
public class MultiThreadMMSkipgramMaxMarginBiDirectional extends MultiThreadWord2Vec{
    public MultiThreadMMSkipgramMaxMarginBiDirectional(int projectionLayerSize, int windowSize,
 ...
 /*************    FOR SECOND MODALITY   ****************/
            ...
            if (negativeSamplesImages != -1 && jPerceptIndex!=-1) {
                ...
                for (int j = 0; j < projectionLayerSize; j++) {
                    ...
                    a1error_temp.set(j,0);
                }
                
                
                //mapping a random words

                for (int l = 0; l < negativeSamplesImages/5 ; l++) {
                    der = new SimpleMatrix(TestConstants.imageDimensions, 1);

                    int random_word_index = rand.nextInt(vocab.getVocabSize()-1)+1;
                    mapped_word_row = (new SimpleMatrix(1,projectionLayerSize,false, weights0[random_word_index]));

                    double cos_neg = MathUtils.cosine(mapped_word_row, image);
                    if (cos-cos_neg >= TestConstants.margin) continue;
                  
                    //calculate error with respect to the cosine
                    der = der.minus(MathUtils.cosineDerivative(mapped_word_row, image));
                    gradient = - (double) (alpha* r);
                    der = der.plus(err_cos_row.scale(k));
                    a1error_temp  = a1error_temp.plus(der.scale(gradient));
                    // Learn weights input -> hidden
                    for (int j = 0; j < projectionLayerSize; j++) {
                        weights0[random_word_index][j] += a1error_temp.get(j, 0);
                        a1error_temp.set(j,0);
                    }
                }

...
public class MultiMmSkipNGramWithMappingMaxMargin extends MultiThreadWord2Vec {
    public MultiMmSkipNGramWithMappingMaxMargin(int projectionLayerSize, int windowSize,
    ...
    public void trainSentence(int[] sentence) {
        ...
        for (int wordPosition = 0; wordPosition < sentence.length; wordPosition++) {
            ...
            if (negativeSamplesImages != -1 && jPerceptIndex!=-1) {
                ...
                a2error = new SimpleMatrix(tmpMatrix.numRows(),tmpMatrix.numCols());
                SimpleMatrix der = new SimpleMatrix(TestConstants.imageDimensions, 1);
                double gradient=0;
                mmWordsPerRun++;
                
                //mapping word
                SimpleMatrix mapped_word_row = (new SimpleMatrix(1,projectionLayerSize,false, weights0[wordIndex])).mult(tmpMatrix);
                ...
                a1error_temp  = a1error_temp.plus(tmpMatrix.mult(der).scale(gradient));

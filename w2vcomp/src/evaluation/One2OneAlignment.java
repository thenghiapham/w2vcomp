...
public class One2OneAlignment
{
    ...
    public One2OneAlignment(double[][] costMatrix)
    {
    ...
    public static void main(String[] args)
    {
        ...
        ArrayList<String> test_elements = new ArrayList<String>(common_elements);
        
        
        double[][] cost = new double[test_elements.size()][test_elements.size()];
        for (int i = 0; i < test_elements.size(); i++)
        {
            for (int j = 0; j < test_elements.size(); j++)
            {
                cost[i][j] = 1-wordSpace.getSim(test_elements.get(i), test_elements.get(j), visionSpace);
            }
        }
        
        One2OneAlignment hbm = new One2OneAlignment(cost);
        int[] result = hbm.execute();
        //System.out.println("Bipartite Matching: " + Arrays.toString(result));
        int correctTranslations = 0;
        for (int i=0;i<result.length;i++){
            System.out.println(test_elements.get(i)+" ==> "+test_elements.get(result[i]));
            if (i==result[i]){
                correctTranslations++;
            }
        }
        System.out.println(correctTranslations+" correct translations ("+100 * (correctTranslations/(double) result.length)+"%)");

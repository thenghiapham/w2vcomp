...
public class Ccg2SurfaceString {
    ...
    public static void preprocessCorpus(BufferedReader reader, BufferedWriter writer) throws IOException{
        ...
        while (line != null) {
            ...
            if ("</ccg>".equals(line)) {
                inTree = false;
                String simpleTreeString = getSurfaceString(treeString);
    ...
    public static String getSurfaceString(String xmlString) {
        try {
            CcgTree ccgTree = CcgTree.parseTreeFromCcgXml(xmlString);
            return ccgTree.getSurfaceString(WordForm.LEMMA_POS);

...
public class PhraseSoftmaxTest {
    ...
    public void testSimple() {
        ...
        ArrayList<SimpleMatrix> numGrads = w2v.computeNumericGradients(inputPhrase, compositionMatrix, softmaxWeight, softmaxValue);
        for (int i = 0; i < 3; i++) {
            assertArrayEquals(grads.get(i).getMatrix().getData(), numGrads.get(i).getMatrix().getData(), 1e-7);
        }
        double value = Math.log(1 - sigmoidTable.getSigmoid(2)) + Math.log(sigmoidTable.getSigmoid(1));
